...
def select(inputt):
    print(f"Dropdown selection changed to: {inputt}")
    if inputt == 'All Sites':
        ...
        # Ensure both "Failure" (0) and "Success" (1) appear
        new_df = spacex_df[spacex_df["Launch Site"] == inputt]["class"].value_counts().reindex([0, 1], fill_value=0).to_frame()
        new_df.reset_index(inplace=True)
        new_df.columns = ["name", "class"]
        new_df["name"] = new_df["name"].map({0: "Failure", 1: "Success"})  # Rename labels
